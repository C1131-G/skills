...
const destination = join(root, "skills/engineering/install-cibi-rules/assets/cibi");
...
if (check) {
  ...
  if (JSON.stringify(expected) !== JSON.stringify(actual)) {
    throw new Error("Skill assets differ from src; run `npm run sync:skill-assets`.");
  }
  for (const path of expected) {
    if (readFileSync(join(source, path), "utf8") !== readFileSync(join(destination, path), "utf8")) {
      throw new Error(`${path} differs from its skill asset; run \`npm run sync:skill-assets\`.`);
    }
  }
