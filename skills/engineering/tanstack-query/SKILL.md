---
name: tanstack-query
description: TanStack Query — keys, queryOptions, cache, mutations. Use when the project has @tanstack/react-query and the task touches server state. Called by master / tanstack router.
...
Optimistic UI path → also `react-async-ui` / `react-optimistic`. Client UI state → `zustand-state`, not the query cache.
