...
const source = resolve(skillRoot, "assets/cibi");
const arguments_ = process.argv.slice(2);
const targetArgument = arguments_.find((argument) => !argument.startsWith("--"));
const target = resolve(process.cwd(), targetArgument ?? "tools/oxlint/cibi");
const force = arguments_.includes("--force");

if (!existsSync(source)) {
  console.error(`Missing bundled plugin at ${source}. Run \`npm run sync:skill-assets\` in the skills repository.`);
  process.exit(1);
}
...
console.log(`Copied the cibi plugin to ${target}`);
