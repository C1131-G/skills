---
name: nextjs-react-conversion
description: Convert components between Next.js and plain React. Called by master.
disable-model-invocation: true
---
# Next.js ↔ React Conversion Rules
...
|---|---|---|
| Routing/navigation | `next/link`'s `<Link>`, `next/navigation`'s `useRouter`/`usePathname`/`useSearchParams` | TanStack Router's `<Link>`, `useNavigate`, `Route.useSearch()` (see `frontend-architecture-rules.md`) |
| Data fetching (initial/route data) | Server Component fetching directly (`async function Page()`), or a route's Server Action | TanStack Router loader + TanStack Query `queryOptions`/`ensureQueryData` (see `frontend-architecture-rules.md`) |
| Mutations | Server Actions (`"use server"` functions passed to `<form action={...}>`) | TanStack Query `useMutation` calling an API endpoint (see `tanstack-query`) |
...
| Middleware | `middleware.ts` at the edge | No equivalent — becomes actual backend middleware (see `backend-architecture`) or route-level logic in the router |
...
2. **If the component was a Server Component doing data fetching directly** (`async function Page() { const data = await fetchX(); ... }`), that pattern doesn't exist in plain React. Convert the fetch into a TanStack Query `queryOptions` factory and move the actual fetch call into a route loader (`ensureQueryData`) with the component reading it via `useSuspenseQuery` — see `frontend-architecture-rules.md` for the full pattern. Don't just wrap the fetch in `useEffect` — that reintroduces exactly the waterfall/loading-state problem the `evil-effect` skill flags.
...
- **After converting**: re-run the other relevant skills in this set on the result — `frontend-architecture-rules.md` if converting into a TanStack Router app, `code-quality` and `typescript-rules.md` regardless of direction — since a mechanical conversion can leave the file in a state that violates rules that were fine before (e.g. a route file that grew past its wiring-only scope during the port).
