...
description: Sonner toasts — when to toast, a11y, motion. Called by master.
...
- **Silent background failures the user can't act on** — don't toast an error the user has no way to respond to; log it instead (see `structured-logging`) and only surface it if there's something the user can actually do.
...
- **Respect `prefers-reduced-motion` here too** (same principle as `react-async-ui`'s View Transitions rule) — fall back to a simple, fast opacity-only fade with no spring/scale/position movement when the user has that preference set:
