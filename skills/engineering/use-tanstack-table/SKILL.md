...
description: Use TanStack Table for headless, typed data tables, row models, selection, sorting, filtering, pagination, and server-driven table state.
---
# use-tanstack-table

Use this skill directly for TanStack Table. Pair it with `use-tanstack-query` when table data or pagination is server-driven.
...
The table's `pagination`/`sorting`/`columnFilters` state is exactly what a data-fetching query key should be built from. Feed that state into a TanStack Query `queryOptions` factory (see `use-tanstack-query`) so a state change naturally produces a new/updated query, instead of a `useEffect` that watches those state values and calls `fetch` manually — that `useEffect` pattern is precisely the kind of effect `audit-react-effects` flags, since TanStack Query's own reactivity to a changed query key already replaces it:
