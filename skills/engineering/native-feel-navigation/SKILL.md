---
name: native-feel-navigation
description: Native-feel nav — View Transitions, Motion. Called by master / react-async-ui.
...
- Keep transition durations short — roughly 200-400ms. Longer reads as sluggish; shorter can feel abrupt. Prefer spring-based easing over fixed-duration linear/ease curves for anything gesture-adjacent (see `react-async-ui`'s and `toast-notifications`' motion-design rules for the same spring-vs-fixed-duration principle applied elsewhere).
